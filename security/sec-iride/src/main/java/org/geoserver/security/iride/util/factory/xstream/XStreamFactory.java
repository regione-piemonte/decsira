 ...
 */
package org.geoserver.security.iride.util.factory.xstream;
...
public class XStreamFactory extends AbstractFactory<XStream> {
    ...
    protected final XStream newInstance() {
        ...
        LOGGER.finest(String.format("XStream: aliasing %d types", this.getAliases().size()));
        for (final Entry<String, Class<?>> entry : this.getAliases().entrySet()) {
            xs.alias(entry.getKey(), entry.getValue());

            LOGGER.finest(String.format("XStream: aliased %s to '%s'", entry.getValue(), entry.getKey()));
        }

        // Allowed types
        final Collection<Class<?>> types = this.getAliases().values();
        LOGGER.finest(String.format("XStream: allowing %d types", types.size()));
        xs.allowTypes(types.toArray(new Class<?>[types.size()]));

        // Custom converters for IRIDE entities
        LOGGER.finest(String.format("XStream: registering %d converters", this.converters.size()));
        for (final ConverterMatcher converter : this.converters) {
            ...
            LOGGER.finest(String.format("XStream: registered %s converter", converter.getClass()));
        }
