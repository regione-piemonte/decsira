...
CREATE VIEW sipra.v_sede AS
    SELECT s.*, c.toponimo as comune, c.sigla_provincia as provincia, si.id_istanza, si.fk_suap, g.geometria
    FROM
...
CREATE VIEW sipra.v_rifiuto AS
    SELECT
        ...
        r.*,
        --a.des_attivita, -- don't join attivita table
    ...
    FROM
        sipra.sipra_dt_t_rifiuto r
            ...
            INNER JOIN sipra.sipra_dt_d_tipo_richiesta tr ON (ia.fk_tipo_richiesta = tr.id_tipo_richiesta)
            -- INNER JOIN sipra.sipra_d_attivita a ON (a.id_attivita = r.id_attivita) -- don't join attivita table
