...
public class MetadataManager {
	...
	try {
	  ...
	  if (elementiInfoBox != null && elementiInfoBox.size() > 0) {
		if (elementiInfoBox.size() == 1) {
		  ...
		  params.put("fk_tipo_funzione", Constants.FUNZIONE_MAPPA_WMS);
		  ...
		  params = new HashMap<String, Object>();
		  params.put("fk_metadato", idMetadato);
		  params.put("fk_tipo_funzione", Constants.FUNZIONE_DOWNLOAD_WFS);
  ...
  public String getMetadata(Integer idCategory, String text, String type) throws MetadataManagerException {
  ...
  public String searchCswMetadata(String txt, String service, int startPosition, int maxRecords) throws CswServiceException, CswAdapterException {
	final String methodName = new Object() {
	}.getClass().getEnclosingMethod().getName();
	...
	if (rtn.size() > 0) {
	  json = getGsonInstance().toJson(rtn.toArray(new CswRecord[rtn.size()]));
	}
